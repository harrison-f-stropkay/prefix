...
- `pre_tokenizer`:
  - Split: since the regex options under the `|` operators cover all cases and since we're using `Isolated`, the split boils down to a ranking of regex matches in this order:
    1. **Clitics**: e.g., `'s`, `'re`
    2. **Words (Letter runs)**: one or more letters (in any language), plus an optional additional single character prefix (which must be non-letter and non-number and non-newline), allowing as one common case a prefix space: `" the"`
...
`tokenizer.json` has a list under `model` called `vocab`. It contains the vocab list _after_ a mapping from the 256 raw bytes to the byte-level "display" alphabet. `decode` shows UTF-8 text; `convert_ids_to_tokens` shows the byte-level token strings.
